...
def load_data():
    ...
    try:
        df_all = pd.read_excel(url, sheet_name="上架完成-空运")  # 全部数据
...
st.subheader("🚢 空派时效深度分析（按区域+物流方式）")
st.divider()
...
time_cols_all = ["到港-提取", "提取-签收", "签收-完成上架"]
...
**🚛 {log_name}**
- 总耗时：{total_time:.2f} 天
- 到港-提取：{log_row['到港-提取']:.1f} 天
- 提取-签收：{log_row['提取-签收']:.1f} 天
