...
if time_col not in df_current.columns or logistics_col not in df_current.columns:
    ...
    if total_orders == 0:
        ...
        if is_all_logistics:
                ...
                if lt in group_data:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            name=f"{lt} - 累计准时率",
                            line=dict(width=2),
                            hovertemplate=f"{lt}<br>时效：%{{x}}天<br>累计准时率：%{{y:.1f}}%<extra></extra>"
                        )
                    )
            ...
            fig.update_layout(
                height=500,
                title="不同计划物流方式 - 累计准时率趋势对比",
                xaxis_title="上架完成-发货时间(天)",
                yaxis_title="累计占比（准时率）(%)",
                yaxis_range=[0, 105],
                legend_title="计划物流方式",
            ...
            # 补充：各物流方式时效分布箱线图（可选，更直观对比）
            st.markdown("#### 📦 各计划物流方式时效分布箱线图")
            fig_box = px.box(
                df_analysis,
                x=logistics_col,
                y=time_col,
                title="不同计划物流方式 - 时效分布对比",
                labels={logistics_col: "计划物流方式", time_col: "上架完成-发货时间(天)"},
                color=logistics_col,
