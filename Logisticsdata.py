...
if month_options and selected_month:
    ...
    required_warehouse_cols = [
        ...
        WAREHOUSE_MONTH_COLUMN_MAPPING["日期列名"],
    ...
    # 第一步：列名校验
    missing_warehouse_cols = [col for col in required_warehouse_cols if col not in warehouse_month_stats.columns]
    if missing_warehouse_cols:
        st.warning(f"仓库分析所需列缺失：{', '.join(missing_warehouse_cols)}，请检查数据列名或映射配置！")
    else:
        # 第二步：提取映射后的列名（方便后续代码复用）
        warehouse_col = WAREHOUSE_MONTH_COLUMN_MAPPING["仓库列名"]
        arrival_ym_col = WAREHOUSE_MONTH_COLUMN_MAPPING["到货年月列名"]
        advance_delay_col = WAREHOUSE_MONTH_COLUMN_MAPPING["提前延期列名"]
        ...
        def split_advance_delay(row):
            advance_delay_str = str(row[advance_delay_col])
        ...
        total_warehouses = df_warehouse_filtered[warehouse_col].nunique()
        total_orders = df_warehouse_filtered["总订单数"].sum() if pd.notna(
            df_warehouse_filtered["总订单数"].sum()) else 0
        ...
        # 生成仓库综合评级数据
        warehouse_summary = []
        for warehouse in df_warehouse_filtered[warehouse_col].unique():
            warehouse_data = df_warehouse_filtered[df_warehouse_filtered[warehouse_col] == warehouse].copy()
        ...
        if latest_month != "无数据":
            ...
            latest_warehouse_count = latest_data[warehouse_col].nunique()
            ...
            if len(valid_months) >= 2:
                ...
                prev_warehouse_count = prev_data[warehouse_col].nunique()
        ...
        st.markdown("---")
        st.markdown("### 仓库明细数据")
        display_cols = ["中文月份", warehouse_col, "总订单数", advance_delay_col, "提前准时订单数", "延期订单数",
                        "准时率(%)", "仓库归类"]
