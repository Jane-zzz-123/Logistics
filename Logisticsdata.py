...
if month_options and selected_month:
    ...
    if "到货日期" in warehouse_month_stats.columns:
        ...
        # 生成「到货年月」（如202509）
        warehouse_month_stats["到货年月"] = warehouse_month_stats["到货日期"].dt.strftime("%Y%m").astype(int)
    ...
    warehouse_month_stats["中文月份"] = warehouse_month_stats["到货年月"].apply(get_chinese_month)

    # ========== 补充3：生成「总订单数」列（与货代一致） ==========
    # 方式1：如果原始数据有「订单号」列，按仓库+年月分组统计订单数
    if "订单号" in warehouse_month_stats.columns:
        # 按仓库、到货年月分组统计总订单数
        order_count = warehouse_month_stats.groupby(["仓库", "到货年月"])["订单号"].nunique().reset_index()
        order_count.columns = ["仓库", "到货年月", "总订单数"]
        # 合并回原数据
        warehouse_month_stats = pd.merge(warehouse_month_stats, order_count, on=["仓库", "到货年月"], how="left")
    # 方式2：如果原始数据有「数量」列，直接用该列作为总订单数
    elif "数量" in warehouse_month_stats.columns:
        warehouse_month_stats["总订单数"] = warehouse_month_stats["数量"]
    # 方式3：如果无订单相关列，默认按行数统计（慎用）
    else:
        order_count = warehouse_month_stats.groupby(["仓库", "到货年月"]).size().reset_index(name="总订单数")
        warehouse_month_stats = pd.merge(warehouse_month_stats, order_count, on=["仓库", "到货年月"], how="left")


    # ========== 补充4：处理「提前/延期（仓库）」列，生成提前准时/延期订单数 ==========
    def split_advance_delay(row):
        """拆分提前/延期（仓库）列，计算提前准时订单数和延期订单数"""
        advance_delay_str = str(row["提前/延期（仓库）"])
        total_orders = row["总订单数"] if pd.notna(row["总订单数"]) else 0

        # 适配常见格式，可根据你的实际数据调整
        if "延期" in advance_delay_str:
            # 提取延期数字：如"延期5单" → 5
            delay_orders = int(''.join([c for c in advance_delay_str if c.isdigit()])) if any(
                c.isdigit() for c in advance_delay_str) else 0
            advance_on_time_orders = max(0, total_orders - delay_orders)  # 避免负数
            ...
            delay_orders = 0
            for part in parts:
                num = int(''.join([c for c in part if c.isdigit()])) if any(c.isdigit() for c in part) else 0
                if "提前" in part or "准时" in part:
                    advance_on_time_orders += num
                elif "延期" in part:
                    delay_orders += num
        elif advance_delay_str.isdigit():
            # 纯数字：默认是延期数
            delay_orders = int(advance_delay_str)
            advance_on_time_orders = max(0, total_orders - delay_orders)
        else:
            # 兜底：无延期
    ...
    # 拆分列
    if "提前/延期（仓库）" in warehouse_month_stats.columns:
        warehouse_month_stats[["提前准时订单数", "延期订单数"]] = warehouse_month_stats.apply(
            split_advance_delay, axis=1
        )
    else:
        st.error("缺少核心列「提前/延期（仓库）」！")
        warehouse_month_stats["提前准时订单数"] = warehouse_month_stats["总订单数"]
        warehouse_month_stats["延期订单数"] = 0

    # ========== 时间筛选（复用货代的start_ym/end_ym） ==========
    ...
    # ========== 计算准时率 + 仓库归类 ==========
    # 计算准时率（避免除以0）
    df_warehouse_filtered["准时率(%)"] = df_warehouse_filtered.apply(
        lambda row: round((row["提前准时订单数"] / row["总订单数"]) * 100, 2) if row["总订单数"] > 0 else 0,
        axis=1
    )
    ...
    # ---------------------- 第二步：仓库整体汇总 & 综合评级（与货代一致） ----------------------
    st.markdown("### 仓库月度表现总结（综合版）")

    total_months = df_warehouse_filtered["中文月份"].nunique()
    total_warehouses = df_warehouse_filtered["仓库"].nunique()
    total_orders = df_warehouse_filtered["总订单数"].sum() if pd.notna(df_warehouse_filtered["总订单数"].sum()) else 0
    avg_overall_rate = round(df_warehouse_filtered["准时率(%)"].mean(), 2) if len(df_warehouse_filtered) > 0 else 0
    ...
    def calculate_warehouse_comprehensive_score(warehouse_data):
        total_orders = warehouse_data["总订单数"].sum() if pd.notna(warehouse_data["总订单数"].sum()) else 0
        total_months = len(warehouse_data)
        # 加权平均准时率（避免除以0）
        weighted_sum = (warehouse_data["准时率(%)"] * warehouse_data["总订单数"]).sum()
        weighted_avg_rate = round(weighted_sum / total_orders, 2) if total_orders > 0 else 0
    ...
    # 异常仓库提醒
    abnormal_warehouses = df_warehouse_comprehensive[df_warehouse_comprehensive["综合评级"] == "异常"]["仓库"].tolist()
    ...
    if latest_month != "无数据":
        ...
        latest_total_orders = latest_data["总订单数"].sum() if pd.notna(latest_data["总订单数"].sum()) else 0
        latest_avg_rate = round(
            (latest_data["提前准时订单数"].sum() / latest_total_orders) * 100, 2
        ) if latest_total_orders > 0 else 0
        ...
        if len(valid_months) >= 2:
            ...
            prev_total_orders = prev_data["总订单数"].sum() if pd.notna(prev_data["总订单数"].sum()) else 0
            prev_avg_rate = round(
                (prev_data["提前准时订单数"].sum() / prev_total_orders) * 100, 2
